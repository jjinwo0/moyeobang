...
export default function CardSlider({dots, account, consumptionProportionByCategory, consumptionProportionByMember, onChange}:CardSliderProps) {
    ...
    // 차트 데이터 만들기
    const chartData = isAccountBalanceByGroup(account) ?
    // 전체일때 
     [  
        {
            totalComsuption : account.totalComsumption,
            comsuptionProportionByCategory : consumptionProportionByCategory,
        },
        {
            totalComsuption : account.totalComsumption,
            comsuptionProportionByCategory : consumptionProportionByCategory,
        ...
        {
            totalMoney : account.totalAmount,
            consumptionProportionByMember : consumptionProportionByMember
        }
    ] :
    // 개인 선택 일때
    [
        {   
            totalComsuption : account.personalTotalSpent,
            comsuptionProportionByCategory : consumptionProportionByCategory,
        },
    ]
        ...
        <div {...swipeHandlers} css={sliderStyle(activeCardIndex)}>
          {isAccountBalanceByGroup(account) ? (
            <>
              ...
              {activeCardIndex === 1 && <ChartCard />}
              {activeCardIndex === 2 && <ChartCard />}
            </>
          ) : (
            <>
              {activeCardIndex === 0 && (
