...
const PlusSelfGoogleMap = forwardRef(({searchLocation}: {searchLocation: string | undefined}, ref) => {
      ...
      if (!selectedPlace && searchLocation && searchLocation.trim() !== '') {
        ...
        service.textSearch(request, (results, status) => {
          if (status === 'OK' && results) {
            ...
            setMarkers(
              nearbyMarkers.filter(marker => marker !== null) as CustomMarker[]
            );
              ...
              service.textSearch(request, (results, status) => {
                if (status === 'OK' && results) {
                  ...
                  if (searchMarkers.length === 1) {
                    map.setCenter(searchMarkers[0]?.position || defaultCenter);
                    map.setZoom(15);
                  } else {
                    const padding = 0;
